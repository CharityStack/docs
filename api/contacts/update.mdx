...
The Update Contact endpoint modifies an existing contact. It uses **partial-update semantics**: only the fields present in the request body are applied, and unknown fields are rejected with `400`. The contact must belong to your merchant account — attempting to update a contact owned by another merchant returns `403 Forbidden`.
...
<ParamField body="firstName" type="string">
  Contact's first name. May be changed but never blanked — omitting it keeps
  the stored value.
</ParamField>

<ParamField body="lastName" type="string">
  Contact's last name. Same rules as `firstName`.
</ParamField>
...
| Status | Code | Meaning |
|--------|------|---------|
| 400 | `validation_failed` | Unknown field, invalid email/phone, duplicate entries, more than one primary, or blanked name. |
