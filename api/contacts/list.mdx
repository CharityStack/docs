---
title: "GET /v1/contacts — list contacts"
sidebarTitle: "List Contacts"
description: "Retrieve a paginated list of all contacts for your merchant account. Optionally filter by email address and page through results with a cursor token."
---
...
Each returned contact carries the same fields as [Get Contact](/api/contacts/get) — including the always-present derived consent enums (`emailConsent`, `smsConsent`, `communicationConsent`) and the read-only giving fields (`hasActiveSubscription`, `firstDonationAt`, `lastDonationAt`, `totalTransactionCount`, `totalTransactionValue`).

Contacts that have been deleted or merged into another contact are not listed.
