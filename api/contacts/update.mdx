...
<ParamField header="Idempotency-Key" type="string" required>
  A unique string you generate for each distinct update. **Reuse the same key
  when you retry** and you get the original result back instead of applying the
  update twice. Required — a request without it is rejected with
  `400 idempotency_key_required`. Same semantics as
  [Create Contact](/api/contacts/create#retrying-safely).
</ParamField>
...
| 404 | — | Contact not found, or it has been deleted or merged into another contact. |
...
| 422 | `idempotency_key_reused` | This `Idempotency-Key` was already used with a **different** request body. Use a new key. |

<Note>
  A request rejected **before** the update is applied — a validation failure,
  or `sms_carrier_locked` — does not consume the idempotency key. Fix the body
