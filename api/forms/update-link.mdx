...
sidebarTitle: "Update Prefilled-Checkout Link"
...
Audit fields (`createdAt`, the original creator) and the link's `active` state are preserved across updates. To flip `active`, use [Activate](/api/forms/toggle-link) or [Deactivate](/api/forms/toggle-link) instead.
