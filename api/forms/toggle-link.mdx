...
sidebarTitle: "Activate / Deactivate Prefilled-Checkout Link"
...
Deactivation is the soft-delete path — the row remains stored and visible via [List Prefilled-Checkout Links](/api/forms/list-links) under `?active=false`, and can be re-enabled by calling activate. For permanent removal, use [Delete Prefilled-Checkout Link](/api/forms/delete-link).
